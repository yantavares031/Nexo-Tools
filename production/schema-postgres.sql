...
CREATE TABLE IF NOT EXISTS agencias (
  ...
  cnpj TEXT NOT NULL,
  "orcamentoAnual" DOUBLE PRECISION NOT NULL DEFAULT 0,
  "boardId" TEXT
);
...
-- Pedido de assinatura de OC (PDF + demanda). Mesmo shape das migrações production/postgres/add-ordens-compra*.sql
...
-- Admin padrão (primeiro acesso em produção). Idempotente: só insere se não existir este e-mail.
-- Trocar e-mail/senha pelo painel após o login. A app compara senha em texto plano (login.use-case).
...
SELECT 'a0000000-0000-4000-8000-000000000001',
       ...
       NULL,
       1
WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'support@nexo.com');
