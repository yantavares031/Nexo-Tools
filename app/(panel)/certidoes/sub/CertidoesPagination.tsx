...
import type { CertidoesFilterParams } from "./CertidoesFilters";

interface CertidoesPaginationProps {
  page: number;
  totalPages: number;
  total: number;
  limit: number;
...
function buildHref(pathname: string, targetPage: number, filters?: CertidoesFilterParams): string {
  const params = new URLSearchParams();
  if (filters?.q) params.set("q", filters.q);
  if (filters?.mes) params.set("mes", filters.mes);
  if (filters?.agenciaId) params.set("agenciaId", filters.agenciaId);
  if (targetPage > 1) params.set("page", String(targetPage));
  const qs = params.toString();
  return qs ? `${pathname}?${qs}` : pathname;
}

export function CertidoesPagination({
  page,
  totalPages,
  total,
  limit,
  filters,
}: CertidoesPaginationProps) {
  ...
  const prevHref = page > 1 ? buildHref(pathname, page - 1, filters) : null;
  const nextHref = page < totalPages ? buildHref(pathname, page + 1, filters) : null;
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-4 sm:flex-row sm:gap-0">
      <p className="text-sm text-slate-600">
        Mostrando {start}–{end} de {total} certidões
      </p>

      <nav className="flex items-center gap-1" aria-label="Paginação de certidões">
        {prevHref ? (
          <Link
            href={prevHref}
        ...
        <div className="flex items-center gap-1">
          {pagesToShow.map((p) => {
            const href = buildHref(pathname, p, filters);
