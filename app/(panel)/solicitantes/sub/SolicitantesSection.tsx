...
export function SolicitantesSection({ solicitantes }: SolicitantesSectionProps) {
  const [search, setSearch] = useState("");

  const filtered = useMemo(() => {
    const term = search.trim().toLowerCase();
    if (!term) return solicitantes;
    return solicitantes.filter(
      (s) =>
        s.nome.toLowerCase().includes(term) ||
        s.unResponsavel.toLowerCase().includes(term)
    );
  }, [solicitantes, search]);
